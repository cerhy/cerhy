...
public class ColumnsDao extends HibernateBaseDao<Columns, Integer>{
	...
	public List<Columns> findInfoByCodeAndUserids(String code, Integer id,
			Integer columnId) {
		Finder f = Finder.create("select bean from Columns bean");
		f.append(" where bean.userId=:userId and bean.uniqueCode=:uniqueCode and bean.columnId!=:columnId");
		f.setParam("uniqueCode", code);
		f.setParam("userId", id);
	...
	public int signOutGroups(Columns cc, CmsUser user) {
		try {
			StringBuilder hql = new StringBuilder("delete CmsJoinGroup bean");
			hql.append(" where bean.joinUserId=:id ");
			hql.append(" and bean.columnsId=:columnsId ");
			Query query = getSession().createQuery(hql.toString());
			query.setParameter("id", user);
			query.setParameter("columnsId", cc);
			query.executeUpdate();
			return 1;
		} catch (Exception e) {
			e.printStackTrace();
